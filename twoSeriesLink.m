...
% and calculates values from runTwoSeriesLinkSim with the 5 values of K:
...
for i=1:+1:99
   currentPercentage = i/100;   % Percentage converted to decimal form for math usage
    ...
    for j = 1:+1:5
        answerList(i,j) = runTwoSeriesLinkSim(K(j),currentPercentage,1000);

        % Calculating results according to formula: K*((2-p)/(1-p)^2) and
        % storing the results in columns 6-10 of answerList
        answerList(i,(j + 5)) = K(j)*((2-currentPercentage)/(1-currentPercentage)^2);
    end
end
...
figureNameList = (["Two Series Link K = 1"; "Two Series Link K = 5"; "Two Series Link K = 15"; "Two Series Link K = 50"; "Two Series Link K = 100"]);
...
figure('Name','Two Series Link All K','NumberTitle','off');
set(gca,'XTickLabel',0:.05:1);
