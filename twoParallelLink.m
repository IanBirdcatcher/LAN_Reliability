...
figureNameList = (["Parallel Link K = 1"; "Parallel Link K = 5"; "Parallel Link K = 15"; "Parallel Link K = 50"; "Parallel Link K = 100"]);
...
% and calculates values from runTwoParallelLinkSim with the 5 values of K:
...
for i=1:+1:99
   ...
   for j = 1:+1:5
        answerList(i,j) = runTwoParallelLinkSim(K(j),currentPercentage,1000);
   end
end
...
figure('Name','Parallel Link All K','NumberTitle','off');
set(gca,'XTickLabel',0:.05:1);
